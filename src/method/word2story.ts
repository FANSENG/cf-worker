...
import { CreateChat } from '../bridge/callLLM';

function assemblePrompt(req: Word2StoryRequest): string {
	return `
...
async function Word2Story(req: Word2StoryRequest, env: Env, ctx: ExecutionContext): Promise<Word2StoryResponse> {
	try {
		const content = await CreateChat(assemblePrompt(req), env.BaseURL, env.APIKEY);
